...
class Results():
    ...
    def set_mode(self, inp): #setting the mode the results section should currently be displayed in
        self.mode = inp

    def display(self, colour): #shows the results section
        ...
        else: #clears the results section to dispplay the route found
            header = Text("Search Results", 47, True, (0, 0, 0), (255, 255, 255), (957,7))
            header.display(text_colour) #displays default title text
        
class Tram_Stop(): #creates and holds information about tram stops
    def __init__(self, name, stop_lines, neighbours, parking, lift, transport_links, zones, rect = None):
        self.name = name #str
        ...
        self.rect = pygame.Rect(rect) if rect is not None else None #tuple or None
        self.active = False
    ...
    def pressed(self): #checks if a tram stop has been pressed
        if self.rect != None:
            ...
            if self.active == True: #shows the message whenever "active" i.e. the button was pressed previously
                if objects[10].get_colour_mode() == True: #setting the text and box colours based on the colour mode
                    text_colour = (0, 0, 0)
                ...
                trams = next_trams(self.name)
                links = ""
                for i in range(0, len(self.transport_links)-1):
                    links = links + self.transport_links[i] + ", "
                links = links + self.transport_links[-1]
                message_text = self.name + "\nNext approaching trams: " + trams + "\nNumber of parking spaces: " + str(self.parking) + "\nLifts: " + self.lift + "\nTransport links: " + links
                while size_found == False: #finding how tall the message_rect should be
                    message_rect = pygame.Rect((0, 0, 200, height))
                    try:
                        rendered_text = render_textrect(message_text, message_font, message_rect, text_colour, box_colour)
                        size_found = True
                    except:
                        height += 1 #increasing height of the rect until the stops fit
                message_rect.bottomleft = (self.rect.bottomleft)
                padding_rect = pygame.Rect((0, 0, 205, height+5)) #larger rect to add text padding
                padding_rect.center = message_rect.center
                outline_rect = pygame.Rect((0, 0, 206, height+6)) #rect to add an outline
                outline_rect.center = message_rect.center
                pygame.draw.rect(win, box_colour, padding_rect, 0, 7) #drawing padding
                pygame.draw.rect(win, outline_colour, outline_rect, 2, 7) #drawing outline
...
class Connector(): #creates the connectors between tram stops
    ...
    def display(self): #displays a connector object
...
class Stop_Input(): #creates the user stop input boxes
    def __init__(self, x_pos, y_pos):
        self.x_pos = x_pos #int
        self.y_pos = y_pos #int
        self.stored_stop =  "" #holds the stop last inputted by the user
        self.inputted_text = "" #holds the string currently inputted by the user
        self.on_screen = "" #holds the string that needs to be displayed in the box on-screen
    ...
    def set_inputted_text(self, new): #sets self.inputted_text to the last string inputted by the user
        self.inputted_text = new
    
    def set_stored_stop(self, new): #stores the stop last inputted by the user
        self.stored_stop = new
    
    def get_active(self):
        return self.active
    
    def set_active(self): #toggles between the active and inactive states
        self.active = not self.active
...
class Line(): #holds information about each Metrolink line
    def __init__(self, name, stops, light_mode_colour, dark_mode_colour):
        ...
        self.dark_mode_colour = dark_mode_colour #tuple of RGB values
...
class Info(): #creates the information icon
    ...
    def pressed(self): #deals with clicks and displays the information box if self.active = True
        ...
        if self.active == True: #shows the message whenever "active" i.e. the button was pressed previously
            message_font = pygame.font.SysFont("calibri", 15)
            message_text = "Click on a box to input a stop. \nClick on the drop-down box to choose when to leave and press 'Calculate' " \
            "to find the shortest route. \nPress the enter key after every text input. \nClick on a tram stop to see the next trams " \
            "leaving from it."
            message_rect = pygame.Rect((16, 615, 250, 108))
            message_rect.bottomleft = (16, 615)
            padding_rect = pygame.Rect((16, 615, 260, 114)) #larger rect to add text padding
            padding_rect.center = message_rect.center
            outline_rect = pygame.Rect((16, 615, 260, 114)) #rect to add an outline
            outline_rect.center = message_rect.center
    ...
    def display(self): #displays the information icon in the right colour mode
        if objects[10].get_colour_mode() == True:
...
class Colour_Toggle(pygame.sprite.Sprite): #creates the colour switching toggle
    def __init__(self, x_pos, y_pos):
...
class Text(): #used to show text on screen
    ...
    def display(self, colour): #displaying the text
...
class Login(): #creates the login button
    def __init__(self):
    ...
    def display(self, colour): #shows the login button
    ...
    #def pressed() #replaces the "Results" section with a login section
...
class Create_Account():
    ...
    def display(self, colour): #shows the create account button
        font = pygame.font.SysFont("calibri", 18, bold = False)
    ...
    #def pressed() #replaces the "Results" section with a account creation section.
...
class Leaving_Time(): #drop down box to select calculation mode and corresponding time
    ...
    def display(self, colour): #draws the main box and the drop-down box if necessary
    ...
    def hover(self, colour): #changing the colour of an option if the mouse is over it
        pos = pygame.mouse.get_pos()
        font = pygame.font.SysFont("calibri", 18, bold = False)
    ...
    def option_click(self): #storing an option if it is clicked
...
class Time_Box(): #takes in the time input for the "Depart at" and "Arrive by" modes
    def __init__(self):
        self.x_pos = 648
        self.y_pos = 130
        self.width = 70
    ...
    def display(self): #displays the input box if required
...
class Calculate(): #creates the calculate button
    def __init__(self):
        self.x_pos = 756
        self.y_pos = 80
        self.width = 77
    ...
    def pressed(self): #checks if the button has been pressed
        action = False
        pos = pygame.mouse.get_pos()
        if self.rect.collidepoint(pos): #checks if the mouse is over the rect
            if pygame.mouse.get_pressed()[0]: #checks if the mouse has left clicked
            ...
            if self.active == True: #checks if a message needs to be shown
                ...
                if successful == True:
                    ...
                    try:
                        ...
                        self.stored_trams = ""
                        print("There is currently a major delay on the network, or works are taking place. Please check tfgm.com for"
                        " more details")
                    draw()
            else:
                draw() #removing the message when clicked again

    def display(self): #displays the button on screen
...
class Prices: #creates the price calculator
    def __init__(self):
        ...
        self.zones_rect1 = pygame.Rect(self.zones_rect.x, self.zones_rect.y + 33, self.zones_rect.w, 10)
        self.zones_rect2 = pygame.Rect(self.zones_rect.x, self.zones_rect.y + 43, self.zones_rect.w, 10)
        self.zones_rect3 = pygame.Rect(self.zones_rect.x, self.zones_rect.y + 53, self.zones_rect.w, 10)
        self.zones_rect4 = pygame.Rect(self.zones_rect.x, self.zones_rect.y + 63, self.zones_rect.w, 10)
        self.zones_rect5 = pygame.Rect(self.zones_rect.x, self.zones_rect.y + 73, self.zones_rect.w, 10)
        self.zones_rect6 = pygame.Rect(self.zones_rect.x, self.zones_rect.y + 83, self.zones_rect.w, 10)
        self.zones_rect7 = pygame.Rect(self.zones_rect.x, self.zones_rect.y + 93, self.zones_rect.w, 10)
        self.zones_rect8 = pygame.Rect(self.zones_rect.x, self.zones_rect.y + 103, self.zones_rect.w, 10)
        self.zones_rect9 = pygame.Rect(self.zones_rect.x, self.zones_rect.y + 113, self.zones_rect.w, 10)
        self.zones_rect10 = pygame.Rect(self.zones_rect.x, self.zones_rect.y + 123, self.zones_rect.w, 10)
        self.inputted_age = "" #pre validation age
        self.age_on_screen = "" #text to be shown on screeen
        ...
        self.duration_rect1 = pygame.Rect(self.duration_rect.x, self.duration_rect.y + 33, self.duration_rect.w, 10)
        self.duration_rect2 = pygame.Rect(self.duration_rect.x, self.duration_rect.y + 43, self.duration_rect.w, 10)
        self.duration_rect3 = pygame.Rect(self.duration_rect.x, self.duration_rect.y + 53, self.duration_rect.w, 10)
        self.duration_rect4 = pygame.Rect(self.duration_rect.x, self.duration_rect.y + 63, self.duration_rect.w, 10)
        self.duration_rect5 = pygame.Rect(self.duration_rect.x, self.duration_rect.y + 73, self.duration_rect.w, 10)
        self.stored_price = "" #last calculated price
        self.price_rect = pygame.Rect(1012, 578, 70, 30)
        self.displayed_price = "" #last displayed price
    ...
    def set_time_active(self): #toggles the time box from active to inactive
    ...
    def display(self): #displays the price calculation box
        ...
        if self.zones_active == True: #checking if the drop-down options need to be displayed
            font = pygame.font.SysFont("calibri", 12, bold = False)
            text1 = font.render("1", True, text_colour)
            pygame.draw.rect(win, box_colour, self.zones_rect1) #displaying the box
            text_rect1 = text1.get_rect(center = self.zones_rect1.center)
            win.blit(text1, text_rect1) #displaying the text

            text2 = font.render("2", True, text_colour) #the above repeated with new text
            pygame.draw.rect(win, box_colour, self.zones_rect2)
            text_rect2 = text2.get_rect(center = self.zones_rect2.center)
            win.blit(text2, text_rect2)

            text3 = font.render("3", True, text_colour) #the same
            pygame.draw.rect(win, box_colour, self.zones_rect3)
            text_rect3 = text3.get_rect(center = self.zones_rect3.center)
            win.blit(text3, text_rect3)

            text4 = font.render("4", True, text_colour) #the same
            pygame.draw.rect(win, box_colour, self.zones_rect4)
            text_rect4 = text4.get_rect(center = self.zones_rect4.center)
            win.blit(text4, text_rect4)

            text5 = font.render("1+2", True, text_colour) #the same
            pygame.draw.rect(win, box_colour, self.zones_rect5)
            text_rect5 = text5.get_rect(center = self.zones_rect5.center)
            win.blit(text5, text_rect5)

            text6 = font.render("2+3", True, text_colour) #the same
            pygame.draw.rect(win, box_colour, self.zones_rect6)
            text_rect6 = text6.get_rect(center = self.zones_rect6.center)
            win.blit(text6, text_rect6)

            text7 = font.render("3+4", True, text_colour) #the same
            pygame.draw.rect(win, box_colour, self.zones_rect7)
            text_rect7 = text7.get_rect(center = self.zones_rect7.center)
            win.blit(text7, text_rect7)

            text8 = font.render("1+2+3", True, text_colour) #the same
            pygame.draw.rect(win, box_colour, self.zones_rect8)
            text_rect8 = text8.get_rect(center = self.zones_rect8.center)
            win.blit(text8, text_rect8)

            text9 = font.render("2+3+4", True, text_colour) #the same
            pygame.draw.rect(win, box_colour, self.zones_rect9)
            text_rect9 = text9.get_rect(center = self.zones_rect9.center)
            win.blit(text9, text_rect9)

            text10 = font.render("1+2+3+4", True, text_colour) #the same
            pygame.draw.rect(win, box_colour, self.zones_rect10)
            text_rect10 = text10.get_rect(center = self.zones_rect10.center)
            win.blit(text10, text_rect10)
        ...
        if self.duration_active == True: #checks if drop-down options currently need to be displayed
            font = pygame.font.SysFont("calibri", 12, bold = False)
            text1 = font.render("Single", True, text_colour)
            pygame.draw.rect(win, box_colour, self.duration_rect1) #displaying the box
            text_rect1 = text1.get_rect(center = self.duration_rect1.center)
            win.blit(text1, text_rect1) #displaying the text

            text2 = font.render("1-day", True, text_colour) #the above repeated with new text
            pygame.draw.rect(win, box_colour, self.duration_rect2)
            text_rect2 = text2.get_rect(center = self.duration_rect2.center)
            win.blit(text2, text_rect2)

            text3 = font.render("7-day", True, text_colour) #the same
            pygame.draw.rect(win, box_colour, self.duration_rect3)
            text_rect3 = text3.get_rect(center = self.duration_rect3.center)
            win.blit(text3, text_rect3)

            text4 = font.render("28-day", True, text_colour) #the same
            pygame.draw.rect(win, box_colour, self.duration_rect4)
            text_rect4 = text4.get_rect(center = self.duration_rect4.center)
            win.blit(text4, text_rect4)

            text5 = font.render("Annual", True, text_colour) #the same
            pygame.draw.rect(win, box_colour, self.duration_rect5)
            text_rect5 = text5.get_rect(center = self.duration_rect5.center)
            win.blit(text5, text_rect5)
        
        pygame.draw.rect(win, box_colour, self.price_rect)

        if self.stored_time != "": #checks if an input is currently stored in time, and if so, displays this
        ...
        elif self.stored_time == "" and self.time_active == False: #if not, "hh:mm" is displayed
            font7 = pygame.font.SysFont("calibri", 18, bold = False)
            text7 = font7.render("hh:mm", True, text_colour)
            text_rect = text7.get_rect(center = self.time_rect.center)
            win.blit(text7, text_rect)
        if self.stored_zones != "": #every other button is checked to show if a value should currently be displayed
        ...
        if self.stored_duration != "":
            ...
            text_rect = text10.get_rect(center = self.duration_rect.center)
            win.blit(text10, text_rect)
        if self.stored_price != "":
            font11 = pygame.font.SysFont("calibri", 20, bold = False)
            text11 = font11.render(self.stored_price, True, text_colour)
            text_rect = text11.get_rect(center = self.price_rect.center)
            win.blit(text11, text_rect)
    
    def calculate_price(self): #finds the price given the inputs
        if (self.stored_age != "" and self.stored_duration != "" and self.stored_time != "" and self.stored_zones !="") or (self.stored_age != "" and self.stored_duration != "" and self.stored_duration != "1-day" and self.stored_zones !=""):
            f = open("fares.txt", "r") #opening file
            read = f.read()
            contents = json.loads(read) #making the file into a list
            f.close()
            for i in range(0, len(contents)): #removing the Â characters
                for j in range(0, len(contents[i])):
                    if contents[i][j][0] != "£" and contents[i][j][0].isnumeric() == False:
                        contents[i][j] = contents[i][j].replace(contents[i][j][0], "")
            for i in contents:
                if i[0] == self.stored_zones: #finding the correct prices for the zones travelled through
                    correct_item = i
            if self.stored_duration == "Single": #finding the price of a single
                standard_price = correct_item[1]
            elif self.stored_duration == "1-day": #finding the price of a 1-day travelcard
                if datetime.datetime.today().weekday() > 4: #checking if the peak or off-peak price should be stored
                    standard_price = correct_item[3]
                else:
                    start_time = datetime.datetime.strptime(self.stored_time,'%H:%M')
                    compare = datetime.datetime.strptime("09:30",'%H:%M')
                    if start_time >= compare:
                        standard_price = correct_item[3]
                    else:
                        standard_price = correct_item[2]
            elif self.stored_duration == "7-day": #finding the price of a 7-day travelcard
                standard_price = correct_item[4]
            elif self.stored_duration == "28-day": #finding the price of a 28-day travelcard
                standard_price = correct_item[5]
            else:
                standard_price = correct_item[6] #finding the price of an annual ticket
            if int(self.stored_age) < 5: #children under 5 travel free
                self.stored_price = "£0"
            elif int(self.stored_age) > 4 and int(self.stored_age) < 17: #children between 5 and 16 travel half price
                standard_price = standard_price[1:]
                price = float(standard_price)/2
                if str(price)[-3] == ".": #checking if a 0 needs to be added for padding
                    self.stored_price = "£"+str(price)
                else:
                    self.stored_price = "£"+str(price)+"0"
            else: #otherwise the standard price applies
                self.stored_price = standard_price
            ...
            if self.zones_active == False and self.stored_price != self.displayed_price: #making sure the text doesn't overlap the drop-down menu
                font = pygame.font.SysFont("calibri", 20, bold = False)
                text = font.render(self.stored_price, True, text_colour)
                text_rect = text.get_rect(center = self.price_rect.center)
                win.blit(text, text_rect)
                self.displayed_price = self.stored_price
                print(self.stored_price)
        else:
            self.stored_price = "" #resetting the stored price

    def time_pressed(self): #checks if the leaving time input has been pressed
        pos = pygame.mouse.get_pos()
        action = False
        if self.time_rect.collidepoint(pos): #checks if the mouse is over the rect
            if pygame.mouse.get_pressed()[0]: #checks if a left click has occured
                self.time_active = not self.time_active
                action = True
            if action == True: #executing code only if a click to active has just occured
    ...
    def age_pressed(self): #checks if the age input has been pressed
        pos = pygame.mouse.get_pos()
        action = False
        if self.age_rect.collidepoint(pos): #checks if the mouse is over the rect
            if pygame.mouse.get_pressed()[0]: #checks if a left click has occured
                self.age_active = not self.age_active
                action = True
            if action == True: #executing code only if a click has just occured
                self.inputted_age = "" #resetting initial conditions
                self.age_on_screen = ""
                self.stored_age = ""
                self.stored_price = ""
                self.display()
    ...
    def zones_pressed(self): #checking if the box has been clicked
        action = False
        pos = pygame.mouse.get_pos()
        if self.zones_rect.collidepoint(pos): #checks if the mouse is over the rect
            if pygame.mouse.get_pressed()[0]: #checks if the mouse has left clicked
                self.zones_active = not self.zones_active
                action = True #stores if a click has happened
        font = pygame.font.SysFont("calibri", 12, bold = False)
        if objects[10].get_colour_mode() == True:
            box_colour = (219, 219, 213)
            ...
            box_colour = (107, 107, 107)
            text_colour = (255, 255, 255)
        if self.zones_active == True: #checks if drop-down options currently need to be displayed
            self.stored_price = ""
            text1 = font.render("1", True, text_colour)
            pygame.draw.rect(win, box_colour, self.zones_rect1) #displaying the box
            text_rect1 = text1.get_rect(center = self.zones_rect1.center)
            ...
            text2 = font.render("2", True, text_colour) #the above repeated with new text for each zone
            pygame.draw.rect(win, box_colour, self.zones_rect2)
            text_rect2 = text2.get_rect(center = self.zones_rect2.center)
            win.blit(text2, text_rect2)
    ...
    def duration_pressed(self): #checking if the duration box has been clicked
        action = False
        pos = pygame.mouse.get_pos()
        if self.duration_rect.collidepoint(pos): #checks if the mouse is over the rect
            if pygame.mouse.get_pressed()[0]: #checks if the mouse has left clicked
                self.duration_active = not self.duration_active
                action = True #stores if a click has happened
        font = pygame.font.SysFont("calibri", 12, bold = False)
        if objects[10].get_colour_mode() == True:
            box_colour = (219, 219, 213)
    ...
    def time_input(self, event): #takes in text input to the time box
        show_message = False #checks if the invalid input message needs to be shown
        if event.key == pygame.K_BACKSPACE:
            self.inputted_time = self.inputted_time[:-1] #keeps all but the last letter inputted
            self.time_on_screen = self.time_on_screen[:-1]
        elif event.key == pygame.K_RETURN: #considering the text input as a whole once enter has been pressed
            correct_input = True
            self.time_active = False
            try:
                if int(self.inputted_time[0:2]) > 23: #rejecting hours above 23
                    correct_input = False
                elif self.inputted_time[2] != ":":
                    correct_input = False
                elif int(self.inputted_time[3:]) > 59: #deals with minutes above 59
                    correct_input = False
            except: #deals with non numeric inputs
                correct_input = False
            if correct_input == False: #displaying a message if a valid time is not inputted
                self.inputted_time = "" #resetting initial conditions
                self.time_on_screen = ""
                if objects[10].get_colour_mode() == True:
                    text_colour = (0, 0, 0)
                else:
                ...
                self.stored_time = self.inputted_time #storing the inputted time if it is a valid input
                self.calculate_price()
                self.display()
        else:
            self.inputted_time += event.unicode #concatenates the last letter inputted to the string
            self.time_on_screen += event.unicode

        if objects[10].get_colour_mode() == True: #displaying the text in light mode
            text_colour = (0, 0, 0)
        else: #displaying the text in dark mode
            text_colour = (255, 255, 255)

        font = pygame.font.SysFont("calibri", 20, bold = False)
        display_text = font.render(self.time_on_screen, True, (text_colour))

        while self.time_rect.w - 4 < display_text.get_width() + 13: #scrolls along the text if there's too much to display
            self.time_on_screen = self.time_on_screen[1:]
            display_text = font.render(self.time_on_screen, True, (text_colour))

        self.display() #redraws the box to cover up previously typed letters
        if show_message == True:
            incorrect_message = pygame.font.SysFont("calibri", 18, bold = False)
            incorrect_text = incorrect_message.render("Invalid Input!", True, text_colour)
            win.blit(incorrect_text, (1101, 583))
        if self.stored_time == "": #avoids drawing a correct input twice
            win.blit(display_text, (self.time_rect.x+13, self.time_rect.y+6))

    def age_input(self, event): #takes in text input to the age box
        show_message = False #checks if the invalid input message needs to be shown
        if event.key == pygame.K_BACKSPACE:
            self.inputted_age = self.inputted_age[:-1] #keeps all but the last letter inputted
            self.age_on_screen = self.age_on_screen[:-1]
        elif event.key == pygame.K_RETURN: #considering the text input as a whole once enter has been pressed
            self.age_active = False
            if self.inputted_age.isnumeric() == True: #checks for a purely numeric age value
                if int(self.inputted_age) < 151: #caps the maximum age
                    correct_input = True
                else:
                    correct_input = False #all other inputs not accepted
            else:
                correct_input = False
            if correct_input == False: #displaying a message if a valid age is not inputted
                self.inputted_age = ""
                self.age_on_screen = ""
                if objects[10].get_colour_mode() == True:
                    text_colour = (0, 0, 0)
                ...
                self.stored_age = self.inputted_age #storing a valid age
                self.calculate_price()
                self.display()
        else:
            self.inputted_age += event.unicode #concatenates the last letter inputted to the string
            self.age_on_screen += event.unicode
        ...
        font = pygame.font.SysFont("calibri", 20, bold = False)
        display_text = font.render(self.age_on_screen, True, (text_colour))

        while self.age_rect.w - 4 < display_text.get_width() + 4: #scrolls along the text if there's too much to display
            self.age_on_screen = self.age_on_screen[1:]
            display_text = font.render(self.age_on_screen, True, (text_colour))
        ...
        if show_message == True:
            ...
            incorrect_text = incorrect_message.render("Invalid Input!", True, text_colour)
            win.blit(incorrect_text, (1101, 583))
        if self.stored_age == "": #avoids drawing a correct input twice
            win.blit(display_text, (self.age_rect.x+4, self.age_rect.y+6)) 

    def zones_hover(self): #checks if the mouse is hovering over one of the zone options
        pos = pygame.mouse.get_pos()
        font = pygame.font.SysFont("calibri", 12, bold = False)
        if objects[10].get_colour_mode() == True:
            box_colour = (179, 179, 177) #setting a darker colour for the selected box in light mode
        ...
        if self.zones_active == True: #checking if the drop-down box is currently displayed
            if self.zones_rect1.collidepoint(pos):
                text1 = font.render("1", True, text_colour)
                pygame.draw.rect(win, box_colour, self.zones_rect1) #displaying the box in the colour indicating hover
                text_rect1 = text1.get_rect(center = self.zones_rect1.center)
                win.blit(text1, text_rect1) #displaying the text

            elif self.zones_rect2.collidepoint(pos):
                text2 = font.render("2", True, text_colour) #the above repeated with new text
                pygame.draw.rect(win, box_colour, self.zones_rect2)
                text_rect2 = text2.get_rect(center = self.zones_rect2.center)
                win.blit(text2, text_rect2)

            elif self.zones_rect3.collidepoint(pos):
                text3 = font.render("3", True, text_colour) #the same
                pygame.draw.rect(win, box_colour, self.zones_rect3)
                text_rect3 = text3.get_rect(center = self.zones_rect3.center)
                win.blit(text3, text_rect3)

            elif self.zones_rect4.collidepoint(pos):
                text4 = font.render("4", True, text_colour) #the same
                pygame.draw.rect(win, box_colour, self.zones_rect4)
                text_rect4 = text4.get_rect(center = self.zones_rect4.center)
                win.blit(text4, text_rect4)

            elif self.zones_rect5.collidepoint(pos):
                text5 = font.render("1+2", True, text_colour) #the same
                pygame.draw.rect(win, box_colour, self.zones_rect5)
                text_rect5 = text5.get_rect(center = self.zones_rect5.center)
                win.blit(text5, text_rect5)

            elif self.zones_rect6.collidepoint(pos):
                text6 = font.render("2+3", True, text_colour) #the same
                pygame.draw.rect(win, box_colour, self.zones_rect6)
                text_rect6 = text6.get_rect(center = self.zones_rect6.center)
                win.blit(text6, text_rect6)

            elif self.zones_rect7.collidepoint(pos):
                text7 = font.render("3+4", True, text_colour) #the same
                pygame.draw.rect(win, box_colour, self.zones_rect7)
                text_rect7 = text7.get_rect(center = self.zones_rect7.center)
                win.blit(text7, text_rect7)

            elif self.zones_rect8.collidepoint(pos):
                text8 = font.render("1+2+3", True, text_colour) #the same
                pygame.draw.rect(win, box_colour, self.zones_rect8)
                text_rect8 = text8.get_rect(center = self.zones_rect8.center)
                win.blit(text8, text_rect8)

            elif self.zones_rect9.collidepoint(pos):
                text9 = font.render("2+3+4", True, text_colour) #the same
                pygame.draw.rect(win, box_colour, self.zones_rect9)
                text_rect9 = text9.get_rect(center = self.zones_rect9.center)
                win.blit(text9, text_rect9)

            elif self.zones_rect10.collidepoint(pos):
                text10 = font.render("1+2+3+4", True, text_colour) #the same
                pygame.draw.rect(win, box_colour, self.zones_rect10)
                text_rect10 = text10.get_rect(center = self.zones_rect10.center)
                win.blit(text10, text_rect10)    

    def duration_hover(self): #checks if the mouse is hovering over one of the ticket duration options
        pos = pygame.mouse.get_pos()
        font = pygame.font.SysFont("calibri", 12, bold = False)
        if objects[10].get_colour_mode() == True:
            box_colour = (179, 179, 177) #setting a darker colour for the selected box in light mode
        ...
        if self.duration_active == True: #checking if the drop-down box is currently displayed
            if self.duration_rect1.collidepoint(pos):
                text1 = font.render("Single", True, text_colour)
                pygame.draw.rect(win, box_colour, self.duration_rect1) #displaying the box
                text_rect1 = text1.get_rect(center = self.duration_rect1.center)
                win.blit(text1, text_rect1) #displaying the text

            elif self.duration_rect2.collidepoint(pos):
                text2 = font.render("1-day", True, text_colour) #the above repeated with new text
                pygame.draw.rect(win, box_colour, self.duration_rect2)
                text_rect2 = text2.get_rect(center = self.duration_rect2.center)
                win.blit(text2, text_rect2)

            elif self.duration_rect3.collidepoint(pos):
                text3 = font.render("7-day", True, text_colour) #the same
                pygame.draw.rect(win, box_colour, self.duration_rect3)
                text_rect3 = text3.get_rect(center = self.duration_rect3.center)
                win.blit(text3, text_rect3)

            elif self.duration_rect4.collidepoint(pos):
                text4 = font.render("28-day", True, text_colour) #the same
                pygame.draw.rect(win, box_colour, self.duration_rect4)
                text_rect4 = text4.get_rect(center = self.duration_rect4.center)
                win.blit(text4, text_rect4)

            elif self.duration_rect5.collidepoint(pos):
                text5 = font.render("Annual", True, text_colour) #the same
                pygame.draw.rect(win, box_colour, self.duration_rect5)
                text_rect5 = text5.get_rect(center = self.duration_rect5.center)
                win.blit(text5, text_rect5)

    def zones_option_click(self): #checks if a zone option has been clicked and stores it
        pos = pygame.mouse.get_pos() #getting the mouse position
        if self.zones_active == True:
            if self.zones_rect1.collidepoint(pos): #checks if the mouse is over the first rect
                if pygame.mouse.get_pressed()[0]: #checks if the mouse has left clicked
                    self.stored_zones = "1" #setting the stored zone to the input
                    self.zones_active = False
                    self.calculate_price()
                    draw() #removing the drop-down box
                ...
                if pygame.mouse.get_pressed()[0]:
                    self.stored_zones = "1+2+3+4"
                    self.zones_active = False
                    self.calculate_price()
                    draw()
    
    def duration_option_click(self): #checks if a duration option has been clicked and stores it
        pos = pygame.mouse.get_pos() #getting the mouse position
        if self.duration_active == True:
            if self.duration_rect1.collidepoint(pos): #checks if the mouse is over the first rect
                if pygame.mouse.get_pressed()[0]: #checks if the mouse has left clicked
                    self.stored_duration = "Single" #setting the stored duration to the input
                    self.duration_active = False
                    self.calculate_price()
                    draw() #removing the drop-down box
                ...
                if pygame.mouse.get_pressed()[0]:
                    self.stored_duration = "Annual"
                    self.duration_active = False
                    self.calculate_price()
                    draw()
            
class TextRectException: #class for render_textrect errors - from the same source
...
def find_zones(route): #find the zones a route travels through
    zones = []
...
def shortest_route(start_stop, end_stop): #dijkstra's implementation to find the shortest route without line changes
    if objects[9].get_mode() != "Leave now":
...
def line_changes(stops_list): #finds line changes given a list of stops on a route
...
def final_time(changes): #finds the trams a user should take, giving the final time with line changes included
    for i in range(0, len(changes)):
        for j in range(0, len(changes[i])):
            if changes[i][j][1] == "Victoria" and changes[i][j][2] == "Grey Line": #compensates for error where only grey line stored
                changes[i][j][2] = ["Grey Line", "Pink Line"]
    if objects[9].get_mode() == "Leave now":
...
def find_pid(section, data): #finds the pid containing the next tram required
    line = find_line_object(section[2])
    ind1 = line.get_stops().index(section[0])
    ind2 = line.get_stops().index(section[1])
    if ind1 < ind2:
...
def next_trams(stop_name): #finds the next trams approaching a stop - to be displayed when a stop is clicked
    data = get_pids()
    pids = []
    for i in data:
        if i["StationLocation"] == stop_name: #checking for if the PID is for the right station
            pids.append(i)
    destinations = []
    times = []
    for j in pids:
        if j["Dest0"] not in destinations and j["Dest0"] != "": #removing PIDs that show the same thing
            destinations.append(j["Dest0"])
            times.append(j["Wait0"])
    trams = ""
    for k in range(0, len(destinations)-1): #creating a string of the next trams approaching
        trams = trams + destinations[k] + " " + times[k] + " mins, "
    trams = trams + destinations[-1] + " " + times[-1] +" mins"
    return trams
...
def get_pids(): #fetches the live data from the api
...
def show_results(route, trams): #shows the results on screen
    if trams == "": #displaying the error message
        ...
        for i in range(0, len(trams)):
            ...
            text3 = font3.render(line3, True, colour)
...
def main(): #creates the program window and checks for user inputs, calling all other functions
    global win
    ...
    while running:
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN: #taking in keyboard input - if any of these is active, they receive the input
                if objects[4].get_active() == True: #checks if the start stop input is active
                    objects[4].text_input(event, objects[10].get_colour_mode())
                if objects[6].get_active() == True: #checks if the end stop input is active
                    objects[6].text_input(event, objects[10].get_colour_mode())
                if objects[12].get_active() == True: #checks if the "Depart at"/"Arrive by" time input is active
                    objects[12].text_input(event, objects[10].get_colour_mode())
                if objects[13].get_time_active() == True: #checks if the price calculator time input is active
                    objects[13].time_input(event)
                if objects[13].get_age_active() == True: #checks if the age input is active
                    objects[13].age_input(event)
            ...
            objects[11].pressed() #checking if the calculate button pressed
            objects[12].pressed() #checking if the "Depart at"/"Arrive by" time input pressed
            objects[13].time_pressed() #checking if the price calculator time input pressed
            objects[13].age_pressed() #checking if the age input pressed
            objects[13].zones_pressed() #checking if the zones drop-down box pressed
            objects[13].zones_hover() #checking if any zone options hovered over
            objects[13].zones_option_click() #checking if any zone options clicked
            objects[13].duration_pressed() #checking if the ticket duration drop-down box pressed
            objects[13].duration_hover() #checking if any duration options hovered over
            objects[13].duration_option_click() #cheecking if any duration options clicked
            objects[13].calculate_price() #checking if the price needs to be calculated

            # for i in tram_stops:
            #     i.pressed() #checking if a stop pressed and its information needs to be shown

        pygame.display.flip() #updating the screen        
    pygame.quit() #closing the program
...
def instantiation(): #instantiates all the objects required
    ...
    #creating all of the tram stop lines
    ...
    brooklands = Tram_Stop("Brooklands", ["Green Line", "Purple Line"], {"Timperley":2, "Sale":2}, 84, "Yes", ["N/A"], ["Zone 3", "Zone 4"], (89, 315, 37, 27))
...
def draw(): #displaying all objects on screen
    light_or_dark = objects[10].get_colour_mode() #checking which colour mode to draw objects using
    if light_or_dark == True:
        win.fill((255, 244, 135))
        map = pygame.image.load(r"C:\Users\archi\OneDrive - Bright Futures Educational Trust\A Level\Computer Science\Project\code\map_lm.png").convert()
        win.blit(map, (3, 175)) #displaying the map
        for i in objects: #iterating through every object and displaying it
            try:
        ...
        win.fill((28, 28, 27))
        map = pygame.image.load(r"C:\Users\archi\OneDrive - Bright Futures Educational Trust\A Level\Computer Science\Project\code\map_dm.png").convert()
        win.blit(map, (3, 175))
        for i in objects:
            try:
                ...
                i.display()
    show_results(objects[11].get_stored_route(), objects[11].get_stored_trams()) #drawing the results onto the results section
